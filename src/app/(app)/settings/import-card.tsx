...
import { importCatalog } from "./import-actions";

export function ImportCard() {
  const [state, formAction, pending] = useActionState(importCatalog, null);
  ...
  return (
    <section className="border border-zinc-200 p-5">
      ...
      <p className="mt-1 max-w-2xl text-sm text-zinc-500">
        Download the template, fill in the Dishes and Meals sheets, then upload
        it. Items are matched by English name — existing ones are updated, new
        ones are added.
      </p>
        ...
        <div className="mt-4 max-w-2xl">
          {state.error ? (
            ...
            <div className="rounded-md border border-emerald-300 bg-emerald-50 px-3 py-2 text-sm text-emerald-900">
              ...
              <p className="mt-0.5">
                Dishes: {state.dishes.inserted} added, {state.dishes.updated}{" "}
                updated · Meals: {state.meals.inserted} added,{" "}
                {state.meals.updated} updated
              </p>
            </div>
          )}
